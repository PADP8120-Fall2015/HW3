...
```
We can reject the null at both the 0.05 and the 0.001 levels because the p(t_{obs} > t_{\alpha=0.05,df=1421.1}). We have strong support to conclude that there is a significant difference in average voter satisifaction based on voter usage of e-voting or traditional voting methods.
*** 
...
```
The difference between the  political knowlege (at levels 1-5) and the average voter satisfaction (positive or  not) was statistically significant  ($F_{obs} > F_{\alpha=0.05,df_1=1,df2=1458}$). We can conclude that on average, voters who have varying levels of political knowledge assess voting satisfaction differently. 
***
...
Level of education is a discrete, numerical variable;the y variable needs to be compatiable. In order to find the correlation between educational level and political information in R, we now have to treat pol_info as a numerical variable instead of as a categorical variable as demonstrated in the previous exercise.
```{r}
?cor()
...
```
The correlation between educational level and political knowledge using Spearman's $/rho$ is 0.3267048 or 0.327.  For every one unit increase in eduucational level, political knowledge may increase 0.327 units.
***
...
```
W can reject the null because the $p_{obs} < p_{0.01}$. Based on these statistics, there is good evidence that there is a significant diffrence in the proportion of vooters whp appproved of President Bush (43) before and after Hurricane Katrina.
...
```
We can be 95% confident that the true value of the average number of yars of piano lessons at SFKWAGAMAWTLTDOSGT is between 3.570 5.630 ($3.570368 < \mu_{piano} < 5.629632$).
...
The $\alpha$ = 0.05 for this t-test. Because the observed p-value is 0.05759731 (or rather 0.058), we barely fail to reject the null and conclude that there is not enough evidence to indicate that there has been an increase in the averaage temperature from 1968 to 2008. Interestingly, if we ran a t-test with an $\alpha$ = 0.10, we could reject the null at this level of signficance, but it is possble thaat we would increase our Type II Error in doing so. That being said, the evidence does seem to point to towards an increase in temperatures that may not yet be statistically significant but could be going forward.
